...
def render_review_page(fund_data, stock_data, index_data, ai_analysis):
    ...
    dd = ((df["net_value"] - peak) / peak * 100).min()
    ...
    with c3:
        st.markdown(f"""<div style="background:#1a1d29;border:1px solid #2a2d3a;border-radius:10px;padding:14px">
            ...
            <div style="font-size:24px;font-weight:600;color:#ff1744">{dd:.2f}%</div>
        </div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""<div style="background:#1a1d29;border:1px solid #2a2d3a;border-radius:10px;padding:14px">
    ...
    # AI 研判
    if ai_analysis:
        st.markdown("<div style='font-size:15px;font-weight:600;color:#e6e9f0;margin:12px 0 6px'>AI 今日研判</div>",
    ...
    # 重仓股表现
    if stock_data:
        st.markdown("<div style='font-size:15px;font-weight:600;color:#e6e9f0;margin:16px 0 8px'>重仓股表现</div>",
                    unsafe_allow_html=True)
        rows = []
        for s in stock_data:
            if "error" not in s:
        ...
        if rows:
            ...
            max_c = max(abs(c) for c in df_stock["涨跌幅"])
            fig3 = go.Figure()
    ...
    st.markdown("<div style='margin-top:16px;font-size:11px;color:#3a3d4a;text-align:center'>数据来源: 天天基金 / 腾讯证券</div>",
                unsafe_allow_html=True)
